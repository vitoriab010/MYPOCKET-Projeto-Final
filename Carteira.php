...
class Carteira {
    ...
    // desfaz o efeito de uma transação antiga no saldo (usado ao editar:
    // primeiro "tira" o valor antigo, depois adicionarTransacao() aplica o novo)
    public function reverterTransacao(string $tipoAntigo, float $valorAntigo): void {
        if ($tipoAntigo === "Entrada") {
            $this->saldo -= $valorAntigo;
        } else {
            $this->saldo += $valorAntigo;
        }
    }
