...
| `UsuarioRepository.php` | Busca ou cria o usuário padrão (o projeto ainda não tem login). |
| `CarteiraRepository.php` | Busca/cria a carteira do usuário e atualiza `saldo_atual`. |
| `CategoriaRepository.php` | Busca uma categoria existente pelo nome+tipo ou cria uma nova (evita duplicar). |
| `TransacaoRepository.php` | Insere uma transação (+ a linha filha em `receita`/`despesa`) e busca todas as transações de uma carteira, já remontando os objetos `Receita`/`Despesa`. |
| `FechamentoMensalRepository.php` | Salva (upsert) o fechamento de um mês/carteira. |
