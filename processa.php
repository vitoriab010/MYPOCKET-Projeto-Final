...
require_once 'CategoriaRepository.php';
...
$categoriaRepositorio = new CategoriaRepository($pdo);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($descricao) && $valor > 0 && !empty($data)) {
        try {
            if ($tipo === 'receita') {
                $categoria = $categoriaRepositorio->buscarOuCriar($nomeCategoria, 'entrada');
                $novaTransacao = new Receita(0, $valor, $data, $descricao, $carteira->getId(), $categoria);
            } else {
                $isDiario = ($tipo === 'diario');
                $categoria = $categoriaRepositorio->buscarOuCriar($isDiario ? 'Diário' : $nomeCategoria, 'saida');
            ...
            $transacaoRepositorio->inserir($novaTransacao, $categoria);
            $carteiraRepositorio->atualizarSaldo($carteira->getId(), $carteira->getSaldo());
